...
//   교차 확인해 필드명 신뢰도가 높다(meta.regularMarketPrice 등).
...
async function fetchYahooIndex(id: string, name: string): Promise<NormalizedQuote> {
  ...
  try {
    ...
    const res = await fetchWithTimeout(url, { 'User-Agent': USER_AGENT, Accept: 'application/json' })
    if (!res.ok) {
    ...
    const parsed = parseYahooChartResult(data.chart?.result?.[0])
